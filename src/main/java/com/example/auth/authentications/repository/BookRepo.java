...
import com.example.auth.authentications.domain.dto.SearchBooksQuery;
...
class BookRepoCustomImpl implements  BookRepoCustom{
    ...
    public List<Book> searchBooks(Page page, SearchBooksQuery query) {
        ...
        if(query.getPublishDateEnd() != null){
            criteriaList.add(Criteria.where("publishDate"). lte(query.getPublishDateEnd()));
        }
        ...
        if (!criteriaList.isEmpty()){
            ...
            operations.add(lookup("authors", "authorIds", "id", "author"));
